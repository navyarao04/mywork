...
class Index(object):
    ...
    def compute_doc_norms(self, index, n_docs, doc_freqs):
        ...
        for i in index:
            for j in index[i]:
                dict_1[j[0]] = dict_1[j[0]] + (((1.0 + math.log10(j[1]))*(math.log10((float(n_docs/doc_freqs[i])))))**2)
        
        for eachindex in dict_1:
            dict_1[eachindex] = math.sqrt(dict_1[eachindex])
    ...
    def compute_doc_lengths(self, index):
        ...
        for eachindex in index:
            for eachindexvalue in index[eachindex]:
                dict_2[eachindexvalue[0]] = (dict_2[eachindexvalue[0]] + eachindexvalue[1])
                
        mean = float(sum(dict_2.values()))/ len(dict_2)  
    ...
    def create_tf_index(self, docs, doc_freqs):
        ...
        for documents in range(0, docs_len):
            ...
            for i in tf_count:
                final_dict[i].append(([temp,(tf_count[i])]))
        return final_dict
