...
class Index(object):
    ...
    def compute_doc_lengths(self, index):
        ...
        """
        ###TODO
        dict_1 = defaultdict(lambda:0.0)
        
        for i in index:
            for j in index[i]:
                dict_1[j[0]] = dict_1[j[0]] + j[1] * j[1]
        
        for d in dict_1:
            dict_1[d] = math.sqrt((dict_1[d]))
    ...
    def create_champion_index(self, index, threshold=10):
        ...
        """
        ###TODO
        dict_2 = defaultdict(lambda:0.0)
        for i in index:
            temp_list_1 = []
            for j in index[i]:
                temp_list_1.append(j)
            dict_2[i] = (sorted(temp_list_1, key=lambda x: x[1], reverse=True))
        
        for k in dict_2:
            dict_2[k] = (dict_2[k][:threshold])
    ...
    def create_tfidf_index(self, docs, doc_freqs):
        ...
        temp = -1
        self.total = docs
        for documents in range(0, docs_len):
            temp = temp + 1
            tf_count = defaultdict(lambda:0.0)
            for doc in docs[documents]:
                tf_count[doc] = tf_count[doc] + 1
            
            for i in tf_count:
                final_dict[i].append(([temp,(1.0 + math.log10(tf_count[i])) * (math.log10(float(docs_len/doc_freqs[i])))]))
        return final_dict
    ...
    def count_doc_frequencies(self, docs):
        ...
        ###TODO
        without_duplicates = []
        dict_4 = defaultdict(lambda: 0.0)
        ind = 0
        
        for doc in docs:
            without_duplicates.append([])
            for term in doc:
                if term not in without_duplicates[ind] :
                    without_duplicates[ind].append(term)
            ind = ind + 1
        
        for doc_1 in without_duplicates:
            for term_1 in doc_1:
                dict_4[term_1] = dict_4[term_1] + 1
    ...
    def query_to_vector(self, query_terms):
        ...
        """
        ###TODO
        dict_5 = defaultdict(lambda:0.0)
        for i in query_terms:
            if i in self.index:
                dict_5[i] = ((math.log10(float(len(self.total))/self.doc_freqs[i])))
        return dict_5
    ...
    def search_by_cosine(self, query_vector, index, doc_lengths):
        ...
        """
        ###TODO
        dict_6 = defaultdict(lambda:0.0)
        for ind in index:
            for val in index[ind]:
                dict_6[val[0]] = dict_6[val[0]] + float((val[1] * query_vector[ind])/doc_lengths[val[0]])
        
        return sorted(list(dict_6.items()), key=lambda x: x[1], reverse=True)
    ...
    def search(self, query, use_champions=False):
        ...
        if use_champions == True:            
            sim = self.search_by_cosine(self.query_to_vector(tokens),self.champion_index,self.doc_lengths)            
        else:
            sim = self.search_by_cosine(self.query_to_vector(tokens),self.index,self.doc_lengths)
